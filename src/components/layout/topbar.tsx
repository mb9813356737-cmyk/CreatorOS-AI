...
export function Topbar() {
  ...
  return (
    <header className="flex h-16 w-full items-center justify-between px-6 border-b border-glass-border/20 bg-surface-0/60 backdrop-blur-md sticky top-0 z-30 select-none">
      {/* Mobile Toggle & Search */}
      <div className="flex items-center gap-4 flex-1">
        <button
          onClick={() => setSidebarMobileOpen(true)}
          className="md:hidden flex h-10 w-10 items-center justify-center rounded-lg border border-glass-border/40 hover:bg-surface-100 hover:border-glass-border text-text-secondary hover:text-text-primary transition-all cursor-pointer"
        >
      ...
      <div className="flex items-center gap-4">
        ...
        {planName === "FREE" && (
          <button
            ...
            className="hidden md:flex h-9 items-center justify-center gap-1.5 px-4 rounded-md bg-linear-to-r from-brand-500 to-accent-400 hover:from-brand-600 hover:to-accent-500 text-white font-bold text-xs shadow-glow-sm hover:shadow-glow-md transition-all cursor-pointer"
          >
            Upgrade
          </button>
        )}
