...
// ─── WebGL Renderer & Pointer Classes ────────────────────────
...
const useShaderBackground = () => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error("WebGL2 shader background failed to initialize:", e);
    }
...
export function ShaderBackground({ className = "absolute inset-0 w-full h-full opacity-30 pointer-events-none z-0" }: { className?: string }) {
  // Completely disabled to remove the background landing animation/firewall while keeping all code intact.
  return null;
}
