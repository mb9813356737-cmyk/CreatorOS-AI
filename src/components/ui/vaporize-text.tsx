...
export default function VaporizeTextCycle({
  ...
  const fontConfig = useMemo(() => {
    ...
    let fontFamily = font.fontFamily || "sans-serif";
    if (typeof window !== "undefined" && fontFamily.startsWith("var(")) {
    ...
    return {
      ...
      font: `${font.fontWeight ?? 400} ${fontSize * globalDpr}px ${fontFamily}`,
    };
...
const renderCanvas = ({
  ...
  let fontFamily = framerProps.font?.fontFamily ?? "sans-serif";
  if (typeof window !== "undefined" && fontFamily.startsWith("var(")) {
    const varName = fontFamily.slice(4, -1).trim();
    const resolved = getComputedStyle(document.documentElement).getPropertyValue(varName).trim();
    if (resolved) {
  ...
  const font = `${framerProps.font?.fontWeight ?? 400} ${fontSize * globalDpr}px ${fontFamily}`;
