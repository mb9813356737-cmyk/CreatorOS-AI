...
class AnimationController {
  ...
  // 设置随机数生成器 (Setup Random Generator)
  ...
  public showProjectedDot(position: Vector3D, sizeFactor: number) {
    ...
    if (position.z > newCameraZ) {
      ...
      // 3D -> 2D投影公式 (3D -> 2D Projection Formula)
  ...
  public render() {
    ...
    // 计算时间参数 (Calculate time parameters)
...
class Star {
  ...
  private rotationDirection: number; // 旋转方向 (Rotation Direction)
  private expansionRate: number; // 扩散速率 (Expansion Rate)
  private finalScale: number; // 最终尺寸比例 (Final Scale)
  
  constructor(cameraZ: number, cameraTravelDistance: number) {
    this.angle = Math.random() * Math.PI * 2;
    this.distance = 30 * Math.random() + 15;
    this.rotationDirection = Math.random() > 0.5 ? 1 : -1;
    this.expansionRate = 1.2 + Math.random() * 0.8; // 增加扩散率 (Increased expansion rate)
    this.finalScale = 0.7 + Math.random() * 0.6; // 最终尺寸 (Final size)
...
export function SpiralAnimation() {
  // Completely disabled to remove dashboard layout spiral canvas animations.
  return null;
}
